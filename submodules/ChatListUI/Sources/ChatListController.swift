...
public class ChatListControllerImpl: TelegramBaseController, ChatListController {
    ...
    private var suppressChatListPeerSelectionUntil: Double = 0.0
    private var isProcessingChatListPeerSelection = false
    ...
    override public func loadDisplayNode() {
        ...
        self.chatListDisplayNode.mainContainerNode.peerSelected = { [weak self] peer, threadId, animated, activateInput, promoInfo in
            Task { @MainActor [weak self] in
                ...
                if self.isProcessingChatListPeerSelection || timestamp < self.suppressChatListPeerSelectionUntil {
                    return
                }
                self.isProcessingChatListPeerSelection = true
                self.suppressChatListPeerSelectionUntil = timestamp + 1.5
                
                let releasePeerSelection = { [weak self] in
                    Queue.mainQueue().after(1.0) {
                        self?.isProcessingChatListPeerSelection = false
                    }
                }
