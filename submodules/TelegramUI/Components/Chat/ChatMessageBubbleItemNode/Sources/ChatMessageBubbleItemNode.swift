...
public class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
    ...
    private var translateButtonNode: ChatMessageShareButton?
    ...
    override public func didLoad() {
        ...
        recognizer.tapActionAtPoint = { [weak self] point in
            if let strongSelf = self {
                ...
                if let translateButtonNode = strongSelf.translateButtonNode, translateButtonNode.frame.contains(point) {
                    return .fail
                }
    ...
    private static func beginLayout(
        ...
        var needsTranslateButton = false
        ...
        if incoming && WhiteGramOtherSettings.current.translationButton && item.associatedData.translateToLanguage == nil && !item.controllerInteraction.expandedTranslationMessageStableIds.contains(item.message.stableId) && !item.message.text.isEmpty {
            let interfaceLanguage = whiteGramNormalizedInterfaceLanguageCode(item.presentationData.strings.baseLanguageCode)
            let (canTranslate, _) = canTranslateText(context: item.context, text: item.message.text, showTranslate: true, ignoredLanguages: [interfaceLanguage])
            needsTranslateButton = canTranslate
        }
        ...
        return (layout, { animation, applyInfo, synchronousLoads in
            return ChatMessageBubbleItemNode.applyLayout(selfReference: selfReference, animation, synchronousLoads,
                ...
                needsTranslateButton: needsTranslateButton,
    ...
    private static func applyLayout(selfReference: Weak<ChatMessageBubbleItemNode>,
        ...
        if needsTranslateButton {
            if strongSelf.translateButtonNode == nil {
                ...
                translateButtonNode.pressed = { [weak strongSelf] in
                    strongSelf?.whiteGramTranslateButtonPressed()
                }
            }
        ...
        let layoutTranslateButton = {
            if let translateButtonNode = strongSelf.translateButtonNode {
                let buttonSize = translateButtonNode.update(presentationData: item.presentationData, controllerInteraction: item.controllerInteraction, chatLocation: item.chatLocation, subject: item.associatedData.subject, message: item.message, account: item.context.account, disableComments: true, isTranslate: true)
                ...
                if let shareButtonNode = strongSelf.shareButtonNode {
                    buttonFrame.origin.x = shareButtonNode.frame.minX
                    buttonFrame.origin.y = shareButtonNode.frame.minY - buttonSize.height - 4.0
                } else if let shareButtonOffset = shareButtonOffset {
                    if incoming {
                ...
                animation.animator.updatePosition(layer: translateButtonNode.layer, position: buttonFrame.center, completion: nil)
                animation.animator.updateBounds(layer: translateButtonNode.layer, bounds: CGRect(origin: CGPoint(), size: buttonFrame.size), completion: nil)
                animation.animator.updateAlpha(layer: translateButtonNode.layer, alpha: (isCurrentlyPlayingMedia || isSidePanelOpen) ? 0.0 : 1.0, completion: nil)
                animation.animator.updateScale(layer: translateButtonNode.layer, scale: (isCurrentlyPlayingMedia || isSidePanelOpen) ? 0.001 : 1.0, completion: nil)
            }
        }
    ...
    override public func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        if let translateButtonNode = self.translateButtonNode, translateButtonNode.frame.contains(point) {
            return translateButtonNode.view.hitTest(self.view.convert(point, to: translateButtonNode.view), with: event)
        }
    ...
    private func whiteGramTranslateButtonPressed() {
        guard let item = self.item else {
            return
        }

        let toLang = whiteGramNormalizedInterfaceLanguageCode(item.presentationData.strings.baseLanguageCode)
        let (_, detectedLanguage) = canTranslateText(context: item.context, text: item.message.text, showTranslate: true, ignoredLanguages: [toLang])
        let fromLang = detectedLanguage.map { normalizeTranslationLanguage($0) }

        item.controllerInteraction.expandedTranslationMessageStableIds.insert(item.message.stableId)
        item.controllerInteraction.requestMessageUpdate(item.message.id, false)

        let _ = (translateMessageIds(context: item.context, messageIds: [item.message.id], fromLang: fromLang, toLang: toLang)
        |> deliverOnMainQueue).startStandalone(completed: { [weak self] in
            ...
            let _ = (item.context.account.postbox.transaction { transaction -> Bool in
                if let message = transaction.getMessage(messageId) {
                    return message.attributes.contains(where: { ($0 as? TranslationMessageAttribute)?.toLang == targetLang })
                } else {
                    return false
                }
            }
            |> deliverOnMainQueue).startStandalone(next: { [weak self] hasTranslation in
                guard let self, let item = self.item else {
                    return
                }
                if !hasTranslation {
    ...
    private func updateAbsoluteRectInternal(_ rect: CGRect, within containerSize: CGSize) {
        ...
        if let translateButtonNode = self.translateButtonNode {
            var translateButtonNodeFrame = translateButtonNode.frame
            translateButtonNodeFrame.origin.x += rect.minX
            translateButtonNodeFrame.origin.y += rect.minY

            translateButtonNode.updateAbsoluteRect(translateButtonNodeFrame, within: containerSize)
        }
