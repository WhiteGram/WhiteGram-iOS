...
func contextMenuForChatPresentationInterfaceState(chatPresentationInterfaceState: ChatPresentationInterfaceState, context: AccountContext, messages: [Message], controllerInteraction: ChatControllerInteraction?, selectAll: Bool, interfaceInteraction: ChatPanelInterfaceInteraction?, readStats: MessageReadStats? = nil, messageNode: ChatMessageItemView? = nil) -> Signal<ContextController.Items, NoError> {
    ...
    if messages.count == 1, let adAttribute = messages[0].adAttribute {
        ...
        if adAttribute.canReport {
            ...
            if chatPresentationInterfaceState.copyProtectionEnabled {
                ...
                if WhiteGramContextMenuSettings.current.isEnabled(.channelCopy) {
            ...
            if let author = message.author, let addressName = author.addressName, WhiteGramContextMenuSettings.current.isEnabled(.channelCopyLink) {
    ...
    |> map { data, updatingMessageMedia, infoSummaryData, appConfig, isMessageRead, messageViewsPrivacyTips, availableReactions, translationSettings, loggingSettings, notificationSoundList, accountPeer -> ContextController.Items in
        ...
        let isChannelMessageContext: Bool
        if let channel = message.peers[message.id.peerId] as? TelegramChannel, case .broadcast = channel.info {
            isChannelMessageContext = true
        } else {
            isChannelMessageContext = false
        }

        func isWhiteGramMessageContextOptionEnabled(fromPrivate privateOption: WhiteGramContextMenuOption, channel channelOption: WhiteGramContextMenuOption) -> Bool {
            return whiteGramContextMenuSettings.isEnabled(isChannelMessageContext ? channelOption : privateOption)
        }

        func appendWhiteGramMessageAction(fromPrivate privateOption: WhiteGramContextMenuOption, channel channelOption: WhiteGramContextMenuOption, item: ContextMenuItem) {
            if isWhiteGramMessageContextOptionEnabled(fromPrivate: privateOption, channel: channelOption) {
                actions.append(item)
            }
        }
        ...
        if let starStatus = data.starStatus {
            ...
            if !isPremiumSticker || chatPresentationInterfaceState.isPremium {
                appendWhiteGramMessageAction(fromPrivate: .privateAddToFavorites, channel: .channelSaveToFavorites, item: .action(ContextMenuActionItem(text: starStatus ? chatPresentationInterfaceState.strings.Stickers_RemoveFromFavorites : chatPresentationInterfaceState.strings.Stickers_AddToFavorites, icon: { theme in
        ...
        if !isPinnedMessages, !isReplyThreadHead, data.canReply, isWhiteGramMessageContextOptionEnabled(fromPrivate: .privateReply, channel: .channelReply) {
        ...
        if !messageText.isEmpty || (resourceAvailable && isImage) || diceEmoji != nil {
            if !isExpired {
                if !isPoll {
                    if !isCopyProtected && isWhiteGramMessageContextOptionEnabled(fromPrivate: .privateCopy, channel: .channelCopy) {
                        actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuCopy, icon: { theme in
                            return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Copy"), color: theme.actionSheet.primaryTextColor)
                        }, action: { _, f in
                ...
                var (canTranslate, _) = canTranslateText(context: context, text: messageText, showTranslate: translationSettings.showTranslate && whiteGramOtherSettings.translationButton, showTranslateIfTopical: showTranslateIfTopical, ignoredLanguages: translationSettings.ignoredLanguages)
        ...
        if data.canEdit && !isPinnedMessages && !isMigrated && isWhiteGramMessageContextOptionEnabled(fromPrivate: .privateEdit, channel: .channelEdit) {
        ...
        if canPin, isWhiteGramMessageContextOptionEnabled(fromPrivate: .privatePin, channel: .channelPin) {
        ...
        if let message = messages.first, message.id.namespace == Namespaces.Message.Cloud, let channel = message.peers[message.id.peerId] as? TelegramChannel, !channel.isMonoForum, !(message.media.first is TelegramMediaAction), !isReplyThreadHead, !isMigrated, whiteGramContextMenuSettings.isEnabled(.channelCopyLink) {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuCopyLink, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Link"), color: theme.actionSheet.primaryTextColor)
            }, action: { _, f in
        ...
        if data.messageActions.options.contains(.forward), isWhiteGramMessageContextOptionEnabled(fromPrivate: .privateForward, channel: .channelForward) {
            if !isCopyProtected {
        ...
        if data.messageActions.options.contains(.report), (!isChannelMessageContext || whiteGramContextMenuSettings.isEnabled(.channelReport)) {
        ...
        if !isReplyThreadHead, (!data.messageActions.options.intersection([.deleteLocally, .deleteGlobally]).isEmpty || clearCacheAsDelete) {
            ...
            let deleteOptionEnabled = isWhiteGramMessageContextOptionEnabled(fromPrivate: title == chatPresentationInterfaceState.strings.Chat_MessageContextMenu_Remove ? .privateRemove : .privateDelete, channel: title == chatPresentationInterfaceState.strings.Chat_MessageContextMenu_Remove ? .channelRemove : .channelDelete)
            if deleteOptionEnabled, let autoremoveDeadline = autoremoveDeadline, !isEditing, !isSending {
            ...
            } else if deleteOptionEnabled, !isUnremovableAction {
        ...
        if !isPinnedMessages, !isReplyThreadHead, data.canSelect, isWhiteGramMessageContextOptionEnabled(fromPrivate: .privateSelect, channel: .channelSelect) {
        ...
        if let message = messages.first, case let .customChatContents(customChatContents) = chatPresentationInterfaceState.subject {
            ...
            case .quickReplyMessageInput:
                ...
                if message.id.namespace == Namespaces.Message.QuickReplyCloud {
                    if data.canEdit && isWhiteGramMessageContextOptionEnabled(fromPrivate: .privateEdit, channel: .channelEdit) {
                        actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_MessageDialogEdit, icon: { theme in
                            return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Edit"), color: theme.actionSheet.primaryTextColor)
                        }, action: { c, f in
                            interfaceInteraction.setupEditMessage(messages[0].id, { transition in
