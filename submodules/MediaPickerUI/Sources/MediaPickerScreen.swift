...
public final class MediaPickerScreenImpl: ViewController, MediaPickerScreen, AttachmentContainable {
    ...
    private class Node: ViewControllerTracingNode, ASGestureRecognizerDelegate {
        ...
        private var cameraPreviewCoverView: UIView?
        ...
        override func didLoad() {
            ...
            if !whiteGramOtherSettings.hideCameraInGallery {
                if case .assets(nil, .default) = controller.subject {
            ...
            if useLegacyCamera {
                ...
                cameraView.pressed = { [weak self, weak cameraView] in
                    if let strongSelf = self, !strongSelf.openingMedia {
                        ...
                        if !enableAnimations && !WhiteGramOtherSettings.current.hideCameraPreviewInGallery {
                ...
                if enableAnimations && !whiteGramOtherSettings.hideCameraPreviewInGallery {
                    cameraView.startPreview()
                }
                ...
                let setupCamera = {
                    ...
                    if !WhiteGramOtherSettings.current.hideCameraPreviewInGallery {
                        camera.startCapture()
                    }
                }
        ...
        private func updateCameraPreviewCover(cameraRect: CGRect? = nil, transition: ContainedViewLayoutTransition = .immediate) {
            let shouldCover = WhiteGramOtherSettings.current.hideCameraPreviewInGallery && (self.cameraView != nil || self.modernCameraView != nil)
            if !shouldCover {
                if let cameraPreviewCoverView = self.cameraPreviewCoverView {
                    self.cameraPreviewCoverView = nil
                    cameraPreviewCoverView.removeFromSuperview()
                }
                return
            }

            let cameraPreviewCoverView: UIView
            if let current = self.cameraPreviewCoverView {
                cameraPreviewCoverView = current
            } else {
                cameraPreviewCoverView = UIView()
                cameraPreviewCoverView.backgroundColor = .black
                cameraPreviewCoverView.isUserInteractionEnabled = false
                self.cameraPreviewCoverView = cameraPreviewCoverView
            }

            if self.modernCameraView != nil {
                if cameraPreviewCoverView.superview !== self.cameraWrapperView {
                    cameraPreviewCoverView.removeFromSuperview()
                    self.cameraWrapperView.addSubview(cameraPreviewCoverView)
                } else {
                    self.cameraWrapperView.bringSubviewToFront(cameraPreviewCoverView)
                }
                transition.updateFrame(view: cameraPreviewCoverView, frame: self.cameraWrapperView.bounds)
            } else if let cameraView = self.cameraView {
                if cameraPreviewCoverView.superview !== self.gridNode.scrollView {
                    cameraPreviewCoverView.removeFromSuperview()
                    self.gridNode.scrollView.addSubview(cameraPreviewCoverView)
                } else {
                    self.gridNode.scrollView.bringSubviewToFront(cameraPreviewCoverView)
                }
                transition.updateFrame(view: cameraPreviewCoverView, frame: cameraRect ?? cameraView.frame)
            }
        }
        ...
        func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
            ...
            if let cameraView {
                if let cameraRect = cameraRect {
                    ...
                    self.updateCameraPreviewCover(cameraRect: cameraRect, transition: transition)
                } else {
            ...
            if case let .noAccess(cameraAccess) = self.state {
                ...
                var hasCamera = cameraAccess == .authorized && !WhiteGramOtherSettings.current.hideCameraInGallery
                var story = false
